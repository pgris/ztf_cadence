...
class PlotCad:
    ...
    def __init__(self, tab=None, color=['green', 'red', 'black'],
                 ls=[':', '-.', '--'], cad_min=0, cad_max=6):
    ...
    def hist_cad(self, all_band=False):
        ...
        for cad in self.n:
            for band in ['cad_ztfg', 'cad_ztfr', 'cad_ztfi']:
                ...
                ratio.append(
                    (tab_cad['healpixID'].count()/self.tab['healpixID'].count()))
                cad_.append(cad+0.5)
                band_.append(band)
        ...
        for i, df_ in enumerate([df_g, df_r, df_i]):
            ...
            bar = ax1.bar(x=df_['cad']+add[i], height=df_['ratio'],
                          width=0.25, alpha=0.5, color=self.color[i],
                          label='{}'.format(df_['band'].unique()[0]))

            p, = ax1.plot(df_['cad']+add[i], df_['accumulate_ratio'],
                          ls=self.ls[i], marker='o', color=self.color[i],
                          label=labels[i])

        # leg = ax1.legend(
        #    title='Accumulate ratio ("ac")\n& cadence :', loc='best')

        leg = ax1.legend(loc='best')

        ax1.set_xlabel('Cadence')
        # ax1.set_ylabel(r'$N_{SN}/N_{total}$')
        ax1.set_ylabel(r'Pixel fraction')
        ...
        if all_band:
            ...
            band_ = []
            for cad in n:
                mask = (self.tab['cad_all'] >= cad) & (
                    self.tab['cad_all'] < cad+1)
                tab_cad = self.tab[mask]
                ratio.append(
                    (tab_cad['healpixID'].count()/self.tab['healpixID'].count()))
                cad_.append(cad+0.5)
            ...
            p, = ax1.plot(df['cad'], df['accumulate_ratio'],
                          ls=':', marker='o', label='accumulate ratio')
